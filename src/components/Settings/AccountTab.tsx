...
export function AccountTab({ user }: { user: UserProfile | null }) {
  ...
  const handleUpgrade = async () => {
    const email = paymentEmail || user?.email;
    if (!email || !email.includes('@')) {
      alert('유효한 이메일 주소를 입력해주세요. 라이선스 키가 이메일로 전송됩니다.');
      return;
    }
  ...
  return (
    <div style={{ animation: 'fadeIn 0.2s ease-out', paddingBottom: '20px' }}>
      ...
      {/* 1. Pro Upgrade Section (The "One-Click" focus) */}
      <div style={{ 
        padding: '28px', 
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          {user?.tier !== 'pro' ? (
            <div style={{ background: 'rgba(255,255,255,0.03)', padding: '20px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
               <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, marginBottom: '8px', color: 'var(--text-secondary)' }}>라이선스 키를 받을 이메일</label>
               <div style={{ display: 'flex', gap: '8px' }}>
                  <input 
                    type="email" 
                    placeholder="your@email.com"
                    value={paymentEmail || user?.email || ''}
                    onChange={e => setPaymentEmail(e.target.value)}
                    style={{ flex: 1, padding: '12px 16px', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '14px', outline: 'none' }}
                  />
                  <button 
